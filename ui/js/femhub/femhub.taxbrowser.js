...
FEMhub.Taxbrowser = Ext.extend(FEMhub.Window, {
    toolbar: null,
    foldersTree: null,
    worksheetsGrid: null,
    ...
    constructor: function(config) {
        ...
        Ext.apply(config, {
            ...
            iconCls: 'femhub-taxbrowser-icon',
            layout: 'border',
            tbar: this.toolbar,
            bbar: this.statusbar,
        ...
        FEMhub.Taxbrowser.superclass.constructor.call(this, config);
    },
    ...
    initToolbar: function() {
        return new Ext.Toolbar({
            ...
            items: [{
                ...
                cls: 'x-btn-text-icon',
                text: 'New Category',
                iconCls: 'femhub-add-taxonomy-icon',
                handler: function() {
                    this.addCategory();
                },
                scope: this,
    ...
    fillFoldersTree: function() {
        ...
        function getFolders() {
            FEMhub.RPC.Category.getFolders({}, function(result) {
                if (result.ok === true) {
    ...
    addCategory: function(node) {
        ...
        Ext.MessageBox.prompt('Add category', 'Enter category name:', function(button, name) {
            if (button === 'ok') {
                if (FEMhub.util.isValidName(name) === false) {
                    ...
                    FEMhub.RPC.Category.create({uuid: node.id, name: name}, function(result) {
                        if (result.ok === true) {
                            ...
                            FEMhub.log("Can't create category");
                        }
                    });
                }
            }
    ...
    deleteCategory: function(node) {
        ...
        if (this.isMyFolders(node)) {
            ...
            Ext.MessageBox.show({
                title: 'Delete folder',
                msg: 'Do you really want to delete selected category and all its contents?',
                buttons: Ext.MessageBox.YESNO,
                icon: Ext.MessageBox.QUESTION,
                fn: function(button) {
                    if (button === 'yes') {
                        FEMhub.RPC.Category.remove({uuid: node.id}, function(result) {
                            if (result.ok === true) {
                                node.remove(true);
                            } else {
                                FEMhub.log("Can't delete category");
                            }
                        });
                    }
                },
    ...
    deleteTaxonomy: function(record) {
        Ext.MessageBox.show({
            title: 'Delete worksheet from taxonomy',
            msg: 'Do you really want to delete selected worksheet?',
            buttons: Ext.MessageBox.YESNO,
            icon: Ext.MessageBox.QUESTION,
            fn: function(button) {
                if (button === 'yes') {
                    FEMhub.RPC.Taxonomy.remove({uuid: record.data.uuid}, function(result) {
                        if (result.ok === true) {
                            this.worksheetsGrid.getStore().remove(record);
                        } else {
                            FEMhub.log("Can't delete taxonomy");
                        }
                    }, this);
                }
            },
    ...
    getWorksheets: function(node) {
        ...
        FEMhub.RPC.Category.getWorksheets({uuid: node.id}, {
            okay: function(result) {
    ...
    addTaxonomy: function() {
        var published = new FEMhub.MyWorksheets({
            buttons: [{
                text: 'Add',
                handler: function() {
                    ...
                    if (worksheet === null) {
                        FEMhub.msg.warning(published, "Select a worksheet first and then click 'Add'.");
                    } else {
                        var node = this.getCurrentNode();

                        var params = {
                            origin_uuid: worksheet.uuid,
                            category_uuid: node.id,
                        };

                        FEMhub.RPC.Taxonomy.add(params, {
                            okay: function(result) {
                                FEMhub.msg.info(this, "'" + result.name + "' was added sucessfully.");
                                this.getWorksheets();
                                published.close();
                            },
                            fail: {
                                'origin-does-not-exist': "Origin worksheet doesn't exist.",
                                'folder-does-not-exist': "Destination category doesn't exist.",
                                'origin-is-not-published': "Origin worksheet isn't published or your's.",
                            },
                            scope: this,
                        });
                    }
...
Ext.reg('x-femhub-taxbrowser', FEMhub.Taxbrowser);

FEMhub.Modules.Taxbrowser = Ext.extend(FEMhub.Module, {
    launcher: {
        text: 'Taxonomy Browser',
        icon: 'femhub-taxbrowser-launcher-icon',
    },
    winCls: FEMhub.Taxbrowser,
});
